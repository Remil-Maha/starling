...
namespace diskann {
  ...
  void PQFlashIndex<T>::load_partition_data(const std::string &index_prefix,
    ...
    diskann::cout << "Partition meta: C: " << C
  ...
  template<typename T>
  void PQFlashIndex<T>::page_search(const T *query1, const _u64 k_search,
    ...
    while (k < cur_list_size && num_ios < io_limit) {
      ...
      if (!frontier.empty()) {
        ...
        for (_u64 i = 0; i < frontier.size(); i++) {
          ...
          if (instr) {
            unsigned page_id = id2page_[id];
            instr->accessed_block_ids.push_back(page_id);
            for (unsigned nid : gp_layout_[page_id]) {
              instr->loaded_node_ids.push_back(nid);
            }
          }
        }
...
#pragma omp critical
      ...
      for (size_t i = 0; i < last_io_ids.size(); ++i) {
        ...
        for (unsigned j = 0; j < vis_size; ++j) {
          // Record neighbor list before compute_and_push_nbrs modifies buffer
          if (instr) {
            unsigned *nbrs_ptr = OFFSET_TO_NODE_NHOOD(vis_cand[j].second);
            unsigned  nn_count = *nbrs_ptr;
            ptrdiff_t buf_off = vis_cand[j].second - sector_buf;
            unsigned  slot = (unsigned) (buf_off / max_node_len);
            unsigned  vis_nid = gp_layout_[pid][slot];
            instr->node_neighbors.push_back(std::make_pair(
                vis_nid,
                std::vector<unsigned>(nbrs_ptr + 1, nbrs_ptr + 1 + nn_count)));
          }
      ...
      for (auto &cached_nhood : cached_nhoods) {
        ...
        // Record neighbor list for cached nhood
        if (instr) {
          instr->node_neighbors.push_back(
              std::make_pair(id, std::vector<unsigned>(cnhood, cnhood + nnr)));
        }
      ...
      for (auto &frontier_nhood : frontier_nhoods) {
        ...
        for (unsigned j = 0; j < gp_layout_[pid].size(); ++j) {
          ...
          if (id == frontier_nhood.first) {
            ...
            compute_extact_dists_and_push(node_buf, id);
            // Record neighbor list for frontier nhood
            if (instr) {
              unsigned *nbrs_ptr = OFFSET_TO_NODE_NHOOD(node_buf);
              unsigned  nn_count = *nbrs_ptr;
              instr->node_neighbors.push_back(std::make_pair(
                  id, std::vector<unsigned>(nbrs_ptr + 1,
                                            nbrs_ptr + 1 + nn_count)));
            }
            compute_and_push_nbrs(node_buf, nk);
          }
    ...
    auto compute_and_push_nbrs = [&](const char *node_buf, unsigned &nk) {
      ...
      if (nbors_cand_size) {
        ...
        for (unsigned m = 0; m < nbors_cand_size; ++m) {
          ...
          Neighbor nn(nbor_id, nbor_dist, true);
          // Return position in sorted list where nn inserted
          auto r = InsertIntoPool(retset.data(), cur_list_size, nn, kicked,
                                  l_search);
          if (cur_list_size < l_search)
            ++cur_list_size;
          // nk logs the best position in the retset that was updated due to
...
#pragma omp critical
  ...
  void PQFlashIndex<T>::page_search_sq(
    ...
    if (mem_L) {
      ...
      std::vector<float>    mem_dists(mem_L);
      std::vector<T *>      res = std::vector<T *>();
      mem_index_->search_with_tags((T *) query, mem_L, mem_L, mem_tags.data(),
                                   mem_dists.data(), nullptr, res);
      compute_and_add_to_retset(
          mem_tags.data(), std::min((unsigned) mem_L, (unsigned) l_search));
    } else {
